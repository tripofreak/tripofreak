  ...
  _supabase.auth.onAuthStateChange((event, session) => {
    ...
    if (event === 'SIGNED_IN') {
      ...
      if (window._authSuccessCallback) {
        window._authSuccessCallback(currentUser);
...
// ── Get access token (with retry for PKCE exchange) ──────────────────
async function getAccessToken(retries = 8, delayMs = 500) {
...
function showLoginModal(onSuccessCallback) {
  ...
  if (!document.getElementById('tf-login-styles')) {
    ...
    style.textContent = `
      ...
      .lm-logo{font-family:'Space Grotesk','Inter',sans-serif;font-size:20px;font-weight:700;color:#0C0C0A;margin-bottom:1rem;display:flex;align-items:center;gap:10px}
      .lm-logo-mark{display:flex;flex-direction:column;gap:3px}
      .lm-logo-sep{width:1px;height:20px;background:rgba(12,12,10,0.15)}
      ...
      .lm-check{color:#2A5C3A;font-size:14px;font-weight:700}
  ...
  modal.innerHTML = `
    ...
    <div class="lm-card">
      ...
      <div class="lm-logo">
        <div class="lm-logo-mark">
          <div style="width:6px;height:6px;border-radius:50%;background:#E8C547"></div>
          <div style="width:6px;height:6px;border-radius:50%;background:rgba(232,197,71,0.4)"></div>
          <div style="width:6px;height:6px;border-radius:50%;background:#C84B2A"></div>
        </div>
      ...
      <ul class="lm-benefits">
        <li><span class="lm-check">✓</span>Every itinerary completely free</li>
        <li><span class="lm-check">✓</span>All trips saved to your account</li>
        <li><span class="lm-check">✓</span>Access from any device, any time</li>
        <li><span class="lm-check">✓</span>Share trips with your travel group</li>
      </ul>
...
async function handleGoogleLogin() {
  ...
  try {
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '⚠️ ' + (err.message || 'Something went wrong. Try again.');
    }
  }
}
...
function updateNavUser(user) {
  ...
  if (user) {
    ...
    if (!document.getElementById('nav-signout')) {
      ...
      signout.style.cssText = 'font-size:11px;color:rgba(255,255,255,0.4);text-decoration:none;cursor:pointer;';
...
// ── Continue Toast ───────────────────────────────────────────────────
function showContinueToast(dest) {
  const existing = document.getElementById('tf-continue-toast');
  if (existing) existing.remove();

  const toast = document.createElement('div');
  toast.id = 'tf-continue-toast';
  toast.style.cssText = `
    position:fixed;bottom:2rem;left:50%;transform:translateX(-50%);
    z-index:9990;background:#0C0C0A;border-radius:12px;
    padding:1rem 1.5rem;box-shadow:0 16px 48px rgba(0,0,0,0.4);
  ...
  if (!document.getElementById('tf-toast-anim')) {
    const style = document.createElement('style');
    style.id = 'tf-toast-anim';
  ...
  toast.innerHTML = `
    <div style="flex:1">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);margin-bottom:3px">Welcome back! Ready to plan</div>
      <div style="font-size:14px;font-weight:600;color:#fff;font-family:'Cormorant Garamond',serif">${dest}</div>
    </div>
    <button onclick="continuePlanning('${dest}')" style="background:#E8C547;color:#0C0C0A;border:none;padding:10px 16px;border-radius:7px;font-size:12px;font-weight:700;cursor:pointer;font-family:'Inter',sans-serif;white-space:nowrap;letter-spacing:0.5px">
      Continue →
    </button>
  `;
...
async function continuePlanning(dest) {
  localStorage.removeItem('tf_pending_dest');
  const toast = document.getElementById('tf-continue-toast');
  if (toast) toast.remove();
  const input = document.getElementById('dest-input');
  if (input) input.value = dest;
  if (window.openForm) openForm(dest);

  // Wait for PKCE token exchange to complete before calling the API
  const token = await getAccessToken();
  if (!token) {
    console.error('[TF] Session token not available after OAuth — re-showing login');
    showLoginModal();
    return;
  }

  if (window.generateItinerary) generateItinerary();
}
